export const categories = [
  {
    title: "Perfumes & Fragrances",
    slug: "perfumes-fragrances",
    image: "/images/brand-free-images/01-cinematic-luxury-perfume-bottle-night.webp",
    summary:
      "Wholesale fragrance sourcing for retailers, distributors, travel retail channels, and duty-free operators requiring reliable access to luxury fragrance portfolios.",
    detail:
      "Imperial Essence Trading LLC supports B2B fragrance sourcing for retailers, distributors, travel retail channels, and duty-free operators seeking consistent category availability. Our approach focuses on premium positioning, dependable supply coordination, and long-term wholesale planning across established fragrance families without presenting brand trademarks on site.",
    capabilities: ["Premium fragrance sourcing", "Travel retail support", "Portfolio planning", "Commercial wholesale"],
    images: [
      "/images/brand-free-images/01-cinematic-luxury-perfume-bottle-night.webp",
      "/images/brand-free-images/02-gold-crowned-luxury-perfume-bottle.webp",
      "/images/brand-free-images/04-black-gold-perfume-bottles-water-splash.webp",
      "/images/brand-free-images/08-gold-black-perfume-bottles-dynamic-splash.webp",
      "/images/brand-free-images/11-four-gradient-luxury-perfume-bottles.webp",
      "/images/brand-free-images/19-lavender-bloom-floral-perfume-display.webp"
    ]
  },
  {
    title: "Niche Fragrances",
    slug: "niche-fragrances",
    image: "/images/brand-free-images/03-ruby-red-perfume-bottle-with-blossoms.webp",
    summary:
      "Selective niche fragrance opportunities for partners looking to strengthen premium discovery, boutique, travel, or destination-led assortments.",
    detail:
      "Niche fragrances require careful positioning, market understanding, and selective access. Imperial Essence Trading LLC helps business partners discuss premium niche fragrance opportunities for boutique retailers, specialty distributors, and travel retail environments.",
    capabilities: ["Selective sourcing", "Boutique channel support", "Premium assortment guidance", "Destination luxury"],
    images: [
      "/images/brand-free-images/03-ruby-red-perfume-bottle-with-blossoms.webp",
      "/images/brand-free-images/05-citrus-pineapple-luxury-perfume-display.webp",
      "/images/brand-free-images/09-lavender-vanilla-floral-perfume-still-life.webp",
      "/images/brand-free-images/13-pink-perfume-white-iris-flowers.webp",
      "/images/brand-free-images/18-rose-crystal-perfume-bottle-closeup.webp"
    ]
  },
  {
    title: "Cosmetics & Makeup",
    slug: "cosmetics-makeup",
    image: "/images/brand-free-images/17-pink-gold-quilted-makeup-compact.webp",
    summary:
      "Premium beauty and makeup supply options for wholesale partners building credible assortments across international retail and distribution channels.",
    detail:
      "Our cosmetics and makeup category is structured for business partners who need dependable sourcing conversations rather than consumer-style product listings. We support wholesale beauty opportunities across international retail, distributor, importer, and buying office needs.",
    capabilities: ["Beauty category planning", "Wholesale supply coordination", "Market-fit guidance", "Volumetric wholesale"],
    images: [
      "/images/brand-free-images/17-pink-gold-quilted-makeup-compact.webp"
    ]
  },
  {
    title: "Skincare",
    slug: "skincare",
    image: "/images/brand-free-images/10-amber-glow-beauty-skincare-collection.webp",
    summary:
      "Advanced dermatological formulations, restorative serums, and ultra-premium skincare lines for luxury retail and travel channels.",
    detail:
      "Imperial Essence Trading LLC supports skincare category inquiries for B2B partners interested in luxury skincare, dermatological formulations, and travel retail opportunities direct from European sourcing channels.",
    capabilities: ["Skincare science", "Dermatological formulations", "Travel retail support", "European clean T2"],
    images: [
      "/images/brand-free-images/06-pastel-skincare-bottles-clean-beauty-display.webp",
      "/images/brand-free-images/07-amber-skincare-dropper-serum-still-life.webp",
      "/images/brand-free-images/10-amber-glow-beauty-skincare-collection.webp"
    ]
  },
  {
    title: "Fashion & Textiles",
    slug: "fashion-textiles",
    image: "/images/brand-free-images/30-white-tweed-runway-fashion-look.webp",
    summary:
      "Fashion wholesale opportunities for business partners seeking premium retail, distributor, and travel retail category extensions.",
    detail:
      "Fashion categories can strengthen retail portfolios when sourced and positioned with discipline. Imperial Essence Trading LLC supports partners exploring premium fashion opportunities for wholesale distribution, retail expansion, or travel retail assortments.",
    capabilities: ["Retail category extension", "Distributor support", "Market suitability review", "Silk & apparel"],
    images: [
      "/images/brand-free-images/12-mens-fashion-shearling-coat-leather-bag.webp",
      "/images/brand-free-images/16-mens-brown-leather-jacket-fashion-portrait.webp",
      "/images/brand-free-images/20-ruffled-skirt-fashion-model-handbag.webp",
      "/images/brand-free-images/24-monochrome-black-white-runway-fashion.webp",
      "/images/brand-free-images/30-white-tweed-runway-fashion-look.webp"
    ]
  },
  {
    title: "Travel Sets",
    slug: "travel-sets",
    image: "/images/brand-free-images/28-luxury-duty-free-shopping-corridor.webp",
    summary:
      "Travel-friendly premium sets designed for duty-free, airport, gifting, hospitality, and destination retail wholesale opportunities.",
    detail:
      "Travel sets are especially relevant for duty-free operators, airport retail, hospitality, and destination-led channels. We support wholesale conversations around compact, premium, and giftable formats that can serve international passengers and retail partners.",
    capabilities: ["Duty-free relevance", "Giftable formats", "Travel retail assortment support", "Airport retail supply"],
    images: [
      "/images/brand-free-images/27-modern-wine-spirits-retail-showroom.webp",
      "/images/brand-free-images/28-luxury-duty-free-shopping-corridor.webp",
      "/images/brand-free-images/29-futuristic-duty-free-retail-interior.webp",
      "/images/brand-free-images/31-luxury-sparkling-wine-bottle-golden-bokeh.webp",
      "/images/brand-free-images/33-premium-dark-wine-bottle-gold-label.webp",
      "/images/brand-free-images/34-burgundy-capped-premium-wine-bottles.webp",
      "/images/brand-free-images/36-luxury-sparkling-wine-bottle-gift-box.webp",
      "/images/brand-free-images/38-warm-lit-premium-spirits-backbar.webp",
      "/images/brand-free-images/39-colorful-premium-spirits-bottle-lineup.webp"
    ]
  },
  {
    title: "Accessories",
    slug: "accessories",
    image: "/images/brand-free-images/26-luxury-handbag-boutique-display.webp",
    summary:
      "Premium accessories sourcing for wholesale partners needing adaptable lifestyle, gifting, and travel retail assortment options.",
    detail:
      "Accessories offer flexible wholesale opportunities across premium retail, duty-free, distributor, and gifting environments. Imperial Essence Trading LLC helps partners evaluate accessory categories with a focus on commercial relevance, availability, and channel positioning.",
    capabilities: ["Lifestyle accessories", "Gifting assortment support", "International sourcing", "Leather goods"],
    images: [
      "/images/brand-free-images/14-brown-leather-chain-hobo-handbag.webp",
      "/images/brand-free-images/15-black-street-style-luxury-handbag.webp",
      "/images/brand-free-images/21-premium-black-leather-belt-silver-buckle.webp",
      "/images/brand-free-images/22-gold-rhinestone-pointed-mule-heels.webp",
      "/images/brand-free-images/23-black-crocodile-texture-top-handle-handbag.webp",
      "/images/brand-free-images/25-black-quilted-handbags-floral-celebration.webp",
      "/images/brand-free-images/26-luxury-handbag-boutique-display.webp"
    ]
  },
  {
    title: "Jewelry",
    slug: "jewelry",
    image: "/images/brand-free-images/44-emerald-sapphire-diamond-necklace-portrait.webp",
    summary:
      "Wholesale jewelry and watchmaking category support for partners exploring premium lifestyle, travel retail, gifting, and high-end retail assortments.",
    detail:
      "The jewelry and fine watchmaking category supports B2B partners interested in premium lifestyle and gifting assortments for retail, travel retail, and distributor channels. Imperial Essence Trading LLC evaluates category availability, presentation standards, and wholesale suitability.",
    capabilities: ["Premium lifestyle sourcing", "Gifting category support", "Fine watchmaking", "Gemstone jewelry"],
    images: [
      "/images/brand-free-images/32-emerald-gemstone-watch-dial-macro.webp",
      "/images/brand-free-images/35-mechanical-watch-movement-closeup.webp",
      "/images/brand-free-images/37-opulent-modern-jewelry-retail-hall.webp",
      "/images/brand-free-images/40-icy-blue-luxury-watch-on-rock.webp",
      "/images/brand-free-images/41-mediterranean-turquoise-statement-necklace.webp",
      "/images/brand-free-images/42-sculptural-diamond-brooch-black-lapel.webp",
      "/images/brand-free-images/43-blue-sapphire-necklace-ring-jewelry-portrait.webp",
      "/images/brand-free-images/44-emerald-sapphire-diamond-necklace-portrait.webp",
      "/images/brand-free-images/45-aquamarine-sapphire-diamond-drop-earrings.webp",
      "/images/brand-free-images/46-art-deco-diamond-necklace-architectural-backdrop.webp",
      "/images/brand-free-images/47-vintage-watchmaking-tools-flat-lay.webp",
      "/images/brand-free-images/48-watch-collection-marble-desk-workspace.webp",
      "/images/brand-free-images/49-luxury-wristwatch-collection-leather-wood.webp"
    ]
  }
];
